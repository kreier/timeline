...
file for that language. For every language that has both, it computes:
  ...
  - how many rows carry machine-translation text in the google, chatgpt,
    gemini and deepl columns (as a % of all translatable rows)
...
AI_COLUMNS = ["google", "chatgpt", "gemini", "deepl"]
AI_COLUMN_LABELS = {
    "google": "Google",
    "chatgpt": "ChatGPT",
    "gemini": "Gemini",
...
def analyze_dictionary(path):
    ...
    rows = [r for r in read_csv_rows(path) if (r.get("tag") or "").strip().lower() not in IGNORED_TAGS]

    group_stats = {}
    for group, tags in TAG_GROUPS.items():
        group_rows = [r for r in rows if (r.get("tag") or "").strip().lower() in tags]
        total = len(group_rows)
        checked = sum(1 for r in group_rows if is_true(r.get("checked")))
        pct = round(100 * checked / total, 1) if total else None
        group_stats[group] = {"checked": checked, "total": total, "pct": pct}

    overall_total = sum(g["total"] for g in group_stats.values())
    overall_checked = sum(g["checked"] for g in group_stats.values())
    overall_pct = round(100 * overall_checked / overall_total, 1) if overall_total else None

    ai_stats = {}
    total_rows = len(rows)
    for col in AI_COLUMNS:
        filled = sum(1 for r in rows if (r.get(col) or "").strip())
        pct = round(100 * filled / total_rows, 1) if total_rows else None
        ai_stats[col] = {"filled": filled, "total": total_rows, "pct": pct}
...
def format_cell_html(pct):
    ...
    return f'<td style="background-color:{color}; text-align:center;">{pct:.1f}%</td>'
...
def build_html_table(matched, supported, results):
    ...
    for key in matched:
        ...
        for group in TAG_GROUPS:
            cells.append(format_cell_html(stats["groups"][group]["pct"]))
...
def build_markdown_table(matched, supported, results):
    ...
    for key in matched:
        ...
        for group in TAG_GROUPS:
            line += format_cell_readme(stats["groups"][group]["pct"])
...
def generate_content():
    ...
    matched, missing_files, orphaned_files = check_consistency(supported, files)

    results = {key: analyze_dictionary(files[key]) for key in matched}

    consistency_section = build_consistency_section(missing_files, orphaned_files)

    intro = (
        f"Tracking {len(matched)} supported language"
        f"{'s' if len(matched) != 1 else ''} against their "
        f"`db/{DICTIONARY_PREFIX}<key>.csv` files.\n"
        "For each of the 6 tag categories (text, bible, B9, A6, wiki, others) "
        "the percentage shows how many entries are marked `checked = TRUE`. "
        "The last 4 columns show how many entries already carry "
        "machine-translated text from that service."
    )

    html_table = build_html_table(matched, supported, results)
    md_table = build_markdown_table(matched, supported, results)
