...
# Scale factor: convert HarfBuzz font units to pixels
upem = hb_face.upem
scale = FONT_PIXEL_SIZE / upem

# Estimate image width from total advance
total_advance_x = sum(pos.x_advance for pos in positions) / 64.0 * scale
...
for info, pos in zip(infos, positions):
    ...
    if bitmap.width > 0 and bitmap.rows > 0:
        ...
        # Apply HarfBuzz offsets (scaled)
        x_offset = (pos.x_offset / 64.0) * scale
        y_offset = (pos.y_offset / 64.0) * scale
    ...
    # Advance pen (scaled)
    x += (pos.x_advance / 64.0) * scale
    y += (pos.y_advance / 64.0) * scale
